...
void set_defaults(SPARC_INPUT_OBJ *pSPARC_Input, SPARC_OBJ *pSPARC) {
    ...
    pSPARC_Input->PrintPdosCohpFlag = 0;
...
void bcast_SPARC_Atom(SPARC_OBJ *pSPARC) {
    ...
    // Just changing psp8 to upf for the pseudopotential name (to be used in PDOS/COHP)
    pSPARC->upfname = (char **)malloc( Ntypes * sizeof(char*) );
    for (int i = 0; i < Ntypes; i++){
        pSPARC->upfname[i] = (char *)malloc(L_PSD * sizeof(char) );
        strcpy(pSPARC->upfname[i], pSPARC->psdName+i*L_PSD);
        char *extension = strstr(pSPARC->upfname[i], ".psp8");
        if (extension) {
            // Replace ".psp8" with ".upf"
            strcpy(extension, ".upf");
        } else {
            fprintf(stderr, "Error: The file extension '.psp8' was not found in '%s'.\n", pSPARC->upfname[i]);
        }
    }
...
void SPARC_copy_input(SPARC_OBJ *pSPARC, SPARC_INPUT_OBJ *pSPARC_Input) {
    ...
    pSPARC->PrintPdosCohpFlag = pSPARC_Input->PrintPdosCohpFlag;
...
void write_output_init(SPARC_OBJ *pSPARC) {
    ...
    fprintf(output_fp,"PRINT_PDOS_COHP: %d\n",pSPARC->PrintPdosCohpFlag);
...
void SPARC_Input_MPI_create(MPI_Datatype *pSPARC_INPUT_MPI) {
    ...
    MPI_Get_address(&sparc_input_tmp.PrintPdosCohpFlag, addr + i++);
