...
/**
 * @brief   This structure type is designed for storing atomic orbitals,
 *          each struct contains nonlocal projectors of atoms of one type.
 */
typedef struct _AO_OBJ {
    int n_orbitals;                  // number of orbitals per atom
    double **Phi;               // orbitals real
    double _Complex **Phi_c;     // orbitals complex
...
typedef struct _SPARC_OBJ{
    ...
    /* AO*/
    ATOM_NLOC_INFLUENCE_OBJ *Atom_Influence_AO;
    ATOM_NLOC_INFLUENCE_OBJ *Atom_Influence_AO_kptcomm;
    ...
    char **upfname;          // pseudopotential file name for each type of atom in UPF format (only used in PDOS, COHP)
