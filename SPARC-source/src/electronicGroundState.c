...
void Calculate_Properties(SPARC_OBJ *pSPARC) {
    ...
    if (pSPARC->mlff_flag < 1){
        ...
        if (rank==0){
            printf("Calculate_electronicGroundState done!\n");
            printf("Starting read_upf_AO!\n");
        }

        read_upf_AO(pSPARC);  // Verified for serial and parallel
        Calculate_SplineDerivAORadFun(pSPARC);
        ...
        if (rank==0){
            for (int ityp = 0; ityp < pSPARC->Ntypes; ityp++){
                n_AO = (pSPARC->AO_rad_str).num_orbitals[ityp];
                for (int n_ao = 0; n_ao < n_AO; n_ao++) {
                    l = (pSPARC->AO_rad_str).Rnl[ityp][n_ao].l;
                    num_total_AO += (2 * l + 1)*pSPARC->nAtomv[ityp];
                }
            }

            snprintf(result, sizeof(result), "%s_AO_info.txt", pSPARC->filename_out);
            fp1 = fopen(result, "w");
            fprintf(fp1, "Number of local atomic orbitals: %d\n", num_total_AO);
            fprintf(fp1, "Names of local atomic orbitals:\n");
            for (int ityp = 0; ityp < pSPARC->Ntypes; ityp++){
                n_AO = (pSPARC->AO_rad_str).num_orbitals[ityp];
                for (int na = 0; na < pSPARC->nAtomv[ityp]; na++){
                    for (int n_ao = 0; n_ao < n_AO; n_ao++) {
                        l = (pSPARC->AO_rad_str).Rnl[ityp][n_ao].l;
                        for (int m = -l; m <=l ; m++){
                            fprintf(fp1, "%s_%s_(%d), ", pSPARC->atomType + ityp*L_ATMTYPE, (pSPARC->AO_rad_str).Rnl[ityp][n_ao].label, m);
                        }
                    }
                }
            }
        ...
        if (rank==0){
            printf("read_upf_AO done!\n");
            printf("Starting GetInfluencingAtoms_AtomicOrbitals!\n");
        }

        ATOM_NLOC_INFLUENCE_OBJ *Atom_Influence_AO = (ATOM_NLOC_INFLUENCE_OBJ *)malloc(sizeof(ATOM_NLOC_INFLUENCE_OBJ) * pSPARC->Ntypes);
        GetInfluencingAtoms_AtomicOrbitals(pSPARC, Atom_Influence_AO, pSPARC->DMVertices_dmcomm, 
            pSPARC->bandcomm_index < 0 ? MPI_COMM_NULL : pSPARC->dmcomm); // Verified for serial and parallel
        
        if (rank==0){
            printf("GetInfluencingAtoms_AtomicOrbitals done!\n");
            printf("Starting CalculateAtomicOrbitals!\n");
        }

        AO_OBJ *AO_str = (AO_OBJ *) malloc( sizeof(AO_OBJ) * pSPARC->Ntypes );
        double ******OIJ_local_images;
        double _Complex ******OIJ_local_images_kpt;
        int ncol = pSPARC->Nband_bandcomm;
        
        double *alpha;
        double _Complex *alpha_kpt;
        int kpt;
        ...
        if (pSPARC->isGammaPoint){
            CalculateAtomicOrbitals(pSPARC, AO_str, Atom_Influence_AO, pSPARC->DMVertices_dmcomm, 
                pSPARC->bandcomm_index < 0 ? MPI_COMM_NULL : pSPARC->dmcomm); // Verified for serial and parallel

            if (rank==0){
                printf("CalculateAtomicOrbitals done!\n");
                printf("Starting Calculate_Overlap_AO!\n");
            }

            Calculate_Overlap_AO(pSPARC, AO_str, Atom_Influence_AO, OIJ_local_images, pSPARC->DMVertices_dmcomm, pSPARC->dmcomm);

            if (rank==0){
                printf("Calculate_Overlap_AO done!\n");
                printf("Starting CalculateAOInnerProductIndex and AO_psi_mult!\n");
            }

            CalculateAOInnerProductIndex(pSPARC);

            for (int spn_i = 0; spn_i < pSPARC->Nspin_spincomm; spn_i++){
                AO_psi_mult(pSPARC, pSPARC->Nd_d, Atom_Influence_AO, 
                      AO_str, ncol, pSPARC->Xorb + spn_i*pSPARC->Nd_d, pSPARC->Nd_d * pSPARC->Nspinor_spincomm, alpha, spn_i, ldo, pSPARC->dmcomm);
            }
        } else {
            for (int kpt = 0; kpt < pSPARC->Nkpts_kptcomm; kpt++) {
                CalculateAtomicOrbitals_kpt(pSPARC, AO_str, Atom_Influence_AO, pSPARC->DMVertices_dmcomm, 
                    pSPARC->bandcomm_index < 0 ? MPI_COMM_NULL : pSPARC->dmcomm); // Verified for serial and parallel

                if (rank==0){
                    printf("CalculateAtomicOrbitals_kpt done!\n");
                    printf("Starting Calculate_Overlap_AO_kpt!\n");
                }

                Calculate_Overlap_AO_kpt(pSPARC, AO_str, Atom_Influence_AO, kpt, OIJ_local_images_kpt, pSPARC->DMVertices_dmcomm, pSPARC->dmcomm);

                if (rank==0){
                    printf("Calculate_Overlap_AO_kpt done!\n");
                    printf("Starting CalculateAOInnerProductIndex and AO_psi_mult!\n");
                }
                ...
                for (int spn_i = 0; spn_i < pSPARC->Nspin_spincomm; spn_i++){
                    AO_psi_mult_kpt(pSPARC, DMnd, Atom_Influence_AO, 
                          AO_str, ncol, pSPARC->Xorb_kpt + kpt*size_k + spn_i*DMnd, DMndsp, alpha_kpt, spn_i, ldo, kpt, pSPARC->dmcomm);
...
void Calculate_electronicGroundState(SPARC_OBJ *pSPARC) {
    ...
    // print final eigenvalues and occupations
    if (pSPARC->PrintPdosCohpFlag == 1) {
        #ifdef DEBUG
        double t1, t2;
        t1 = MPI_Wtime();
        #endif
        print_DOS_COHP(pSPARC);
        #ifdef DEBUG
        t2 = MPI_Wtime();
        if (rank == 0) printf("Time for printing eigenvalues: %.3f ms\n", (t2-t1)*1e3);
        #endif
